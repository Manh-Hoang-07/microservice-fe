...
**Goal:** Chuyển trang chủ vào `(public)` route group để hưởng header/footer streaming đã có, loại bỏ 2 fetch thừa khỏi critical path, thêm Suspense skeleton để user thấy layout ngay lập tức.

**Architecture:** Di chuyển `src/app/page.tsx` → `src/app/(public)/page.tsx`. Bỏ `PublicLayoutWrapper` + `getSystemConfig` + `getPublicMenus` (layout lo). Tạo `AsyncHomepageContent` server component bọc trong `<Suspense fallback={<HomepageSkeleton />}>`— user thấy skeleton ngay khi request đến, content stream vào sau khi `getComicHomepageData` xong.
...
## Task 1: Tạo `(public)/page.tsx` với Suspense streaming

**Files:**
- Create: `src/app/(public)/page.tsx`
- Delete: `src/app/page.tsx`

- [ ] **Step 1: Tạo `src/app/(public)/page.tsx`**
...
git commit -m "perf: move homepage into (public) route group, add Suspense streaming"
```
...
| | Trước | Sau |
|---|---|---|
| **Fetch trên homepage** | `getComicHomepageData` + `getSystemConfig` + `getPublicMenus` | Chỉ `getComicHomepageData` |
| **Khi API chậm** | Màn hình trắng cho đến khi tất cả xong | Skeleton ngay lập tức, content stream vào |
| **Header/Footer** | Chờ cùng data homepage | Stream độc lập qua layout |
| **`getSystemConfig` calls/request** | 2 (page + root layout metadata) | 1 (root layout metadata, `React.cache()` dedup) |
